...
import { calculateCeoDashboard, formatCeoMoney, type CeoMission } from '../lib/dashboard.ts'
import { JosButton, KpiCard, NoticeCard, SectionHeader } from '../ui/index.ts'
import { calculateExecutiveKpis } from '../lib/executiveKpis.ts'
import { formatFinanceMoney } from '../lib/finance.ts'
import { calculateBrandPerformance } from '../lib/brandPerformance.ts'
import { calculateCeoRecommendations } from '../lib/ceoRecommendations.ts'
import { calculateBusinessForecast } from '../lib/businessForecasting.ts'
import { calculateAutomationReport } from '../lib/automationCentre.ts'
...
export function Dashboard({
  ...
  const salesPlan = calculateSalesProfitPlan(items, orders, settings)
  ...
  return (
    <main className="screen ceo-dashboard">
      ...
      <section className="ceo-sales-plan-summary">
        <div><p className="eyebrow">SALES & PROFIT PLAN</p><h2>{formatFinanceMoney(salesPlan.targetRemaining)} remaining</h2><p>{salesPlan.salesRequired} sales required · {salesPlan.targetProgress}% achieved · {salesPlan.confidenceScore}/100 confidence</p></div>
        <div className="ceo-sales-plan-metrics"><span><strong>{formatFinanceMoney(salesPlan.planningAverageProfit)}</strong>profit per sale</span><span><strong>{salesPlan.additionalListingsRequired}</strong>listings needed</span><span><strong>{salesPlan.health.overall}/100</strong>business health</span></div>
        <JosButton variant="secondary" fullWidth onClick={onOpenSalesPlanning}>Open Sales & Profit Plan</JosButton>
      </section>
