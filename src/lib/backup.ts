import type { FinanceState, FinanceTransaction, FinanceTransactionType, InventoryItem, JosSettings, ListingChecklist, ListingPipelineStage, OrderRecord, PhotoChecklist, StockStatus } from '../types/inventory.ts'
import { normaliseAutomationSettings } from './automationCentre.ts'
import { normaliseLaunchCommandSettings } from './launchCommand.ts'
import { normaliseSalesPlanningSettings } from './salesProfitPlanning.ts'
...
export function migrateBackup(input: unknown): JosBackup {
  ...
  const settings: JosSettings = {
    ...
    salesPlanning: normaliseSalesPlanningSettings(rawSettings.salesPlanning),
